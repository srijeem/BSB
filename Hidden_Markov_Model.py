...
def baumwelch(set_X,A,E):
    ...
    def sum_all():
        return [sum(new_A[k].values()) for k in allStates]
    sumOfValues=[]
    sumOfValues=sum_all()
    '''print("sumOfValues for all states using function:" )
    print(sumOfValues)
    print("list of k states using function:")
    print(k)'''
    for k in allStates:
        if k=='B':
            for transition, prob in new_A[k].items():
                new_A[k][transition] = (prob / sumOfValues[0])
        elif k=='D':
            for transition, prob in new_A[k].items():
                new_A[k][transition] = (prob / sumOfValues[1])
        elif k=='L':
            for transition, prob in new_A[k].items():
                new_A[k][transition] = (prob / sumOfValues[2])
            
                
    '''def sum_all_2():
        return [sum(new_A[k].values()) for k in allStates]
    sumOfValues_2=sum_all_2()
    print("sumOfValues for all states:" )
    print(sumOfValues_2)
